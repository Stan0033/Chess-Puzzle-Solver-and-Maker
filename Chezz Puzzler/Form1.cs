...
namespace Chezz_Puzzler
{
    public partial class Form_base : Form
    {
        ...
        readonly Dictionary<char, Image> CharactersAsPieceImages;
        //currently
        ...
        // 
        readonly Dictionary<string, string> ImageToLetter;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (int file = 1; file <= 8; file++)
            {
                for (int rank = 1; rank <= 8; rank++)
                {
                    ...
                    bool isWhite = currentColor != Color_Black_Current;
                    //------------------------------------------------------------------
            ...
            if (e.Button.ToString() == "Left")
            {
                ...
                if (APuzzleIsLoaded == true && CurrentPuzzleIsSolved == false && UserToMove == true)
                {
                    if (Selected_A_Piece == false) // if you click the starting square
                    {
                        ...
                        //----------------------------------------------------------------------
                        if (char.IsUpper(SelectedSquare_Start.PieceName[0]) && ___LABEL_TOMOVE.Text == "Black to move") { return; }
                        if (char.IsLower(SelectedSquare_Start.PieceName[0]) && ___LABEL_TOMOVE.Text == "White to move") { return; }
                        //----------------------------------------------------------------------
                        ...
                        if (CurrentPuzzleIsSolved == false)
                        {
                            ...
                            if (ProposedSolution == CurrentPuzzle_Solutions[CurrentlySolvedPuzzleChapterStep])
                            {
                                ...
                                if (CurrentlySolvedPuzzleChapterStep == CurrentPuzzle_Solutions.Count) // if the puzzle is completed
                                {
                                    ...
                                    ___LABEL_TOMOVE.ToMove = "None"; SetToMoveInButtons(___LABEL_TOMOVE.Text);
                                    CurrenlyOpenedPuzzle_ChaptersSolved = 0;
                                    //------------------------------------------------------------------------------------------------------------------
                                    //------------------------------------------------------------------------------------------------------------------
                                    ...
                                    if (CurrentlySolvingAPuzzleRush)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                ___LABEL_TOMOVE.ToMove = CurrentPuzzle_ToMove[CurrentlySolvedPuzzleChapterStep]; SetToMoveInButtons(___LABEL_TOMOVE.Text);
                                            }
                                        }
                                    }
                                    //------------------------------------------------------------------------------------------------------------------
                                ...
                                else // if the  the answer is correct
                                {
                                    ...
                                    ___LABEL_TOMOVE.ToMove = CurrentPuzzle_ToMove[CurrentlySolvedPuzzleChapterStep - 1]; SetToMoveInButtons(___LABEL_TOMOVE.Text);
